    ...
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 2, py: 1, borderBottom: 1, borderColor: 'divider', position: 'sticky', top: 0, bgcolor: 'background.paper', zIndex: 10 }}>
      {viewMode === 'list' && (
        <ToggleButtonGroup size="small" value={filter} exclusive onChange={(_, v) => v && onFilterChange(v)}
          sx={{ '& .MuiToggleButton-root': { px: 1.5, py: 0.5, fontSize: '0.8rem', textTransform: 'none', '&.Mui-selected': { bgcolor: 'primary.main', color: 'white', '&:hover': { bgcolor: 'primary.dark' } } } }}>
      ...
      <TextField
        ...
        sx={{ minWidth: 200, '& .MuiInputBase-root': { fontSize: '0.875rem', borderRadius: 2 } }}
      />

      <Box sx={{ flex: 1 }} />
      <ToggleButtonGroup size="small" value={viewMode} exclusive onChange={(_, v) => v && onViewModeChange(v)}
        sx={{ '& .MuiToggleButton-root': { px: 1, py: 0.5, '&.Mui-selected': { bgcolor: 'primary.main', color: 'white', '&:hover': { bgcolor: 'primary.dark' } } } }}>
      ...
      {!selectMode && showUpload && (
        <Button variant="contained" color="warning" size="small" startIcon={<CloudUploadIcon />} onClick={onUpload} aria-label="Subir archivos">
          Subir
        </Button>
      )}
      ...
      <Button
        size="small"
        color={selectMode ? 'error' : 'primary'}
        variant="outlined"
